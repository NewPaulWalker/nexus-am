...
void setTrigger(){
    asm volatile(
        ...
        "csrw tdata1, %0\n\t"
        :
        :"r"((TDATA1_TYPE << (__riscv_xlen - 4)) | ICOUNT_S | (10 << ICOUNT_COUNT_OFFSET))
        :
    );
}
...
__attribute__((aligned(4))) void __am_asm_scode(void){
asm volatile(
    "li t1, 0\n\t"
    "addi t1, t1, 1\n\t"
    "addi t1, t1, 1\n\t"
...
int main() {
    ...
    asm volatile(
        ...
        "mret"
        :
        :"r"(__am_asm_strap), "r"(__am_asm_scode), "r"(0x800), "r"(0x2), "r" (1<<3)
        );    
